...
| `20260927000000_password_reset` | `User.sessionVersion`, `User.passwordChangedAt`; `PasswordResetToken` (SHA-256 `tokenHash` unique, `expiresAt`, `usedAt`, `issuedById` → User RESTRICT, user CASCADE); `PasswordResetRequest` (email hash + time, for rate limiting) | additive |
| `20260927010000_revoked_sessions` | `RevokedSession` (signed-out session ids until the JWT's 30-day lifetime ends) | additive |
...
The 19 migrations up to `20260926000000` were verified end-to-end twice:
...
The two 2026-09-26 migrations (password reset, revoked sessions) are
purely additive:
- all 21 migrations applied to an empty database in 1.8 s, and
  `migrate status` reported "up to date";
- both applied cleanly to the populated development database;
- both applied cleanly to the test database used by the 496 tests.

No existing row is touched: `sessionVersion` defaults to 0, which is what
tokens issued earlier count as.
